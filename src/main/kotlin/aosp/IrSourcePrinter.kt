// https://cs.android.com/androidx/platform/frameworks/support/+/androidx-main:compose/compiler/compiler-hosted/src/main/java/androidx/compose/compiler/plugins/kotlin/lower/IrSourcePrinter.kt;drc=e44d8178e8c057e9cd8e0ba2ba4744796295f0f2

@file:Suppress("unused", "KotlinConstantConditions", "MemberVisibilityCanBePrivate", "RedundantOverride")
...
//          println("// FILE: ${declaration.fileEntry.name}")
  ...
  override fun visitCall(expression: IrCall) {
    ...
    if (isOperator) {
      ...
      when (name) {
        ...
        "iterator", "hasNext", "next", "noWhenBranchMatchedException" -> {
          (expression.dispatchReceiver ?: expression.extensionReceiver)?.print()
          print(".")
          print(opSymbol)
          print("()")
    ...
    when {
      ...
      isIf -> {
        ...
        expression.branches.forEachIndexed { index, branch ->
          val isElse = index == expression.branches.size - 1 && (branch.condition as? IrConst<*>)?.value == true
          when {
            index == 0 -> {
              print("if (")
            ...
            isElse -> {
              if (singleLine) print(" else ")
              else println("} else {")
            }
            else -> {
              if (singleLine) print(" else if (")
              else print("} else if (")
              branch.condition.print()
              if (singleLine) print(") ")
              else println(") {")
  ...
  private fun IrExpression.isLastStatementIn(function: IrFunction) =
  ...
  override fun visitReturn(expression: IrReturn) {
    ...
    if (value.type.isUnit() && value is IrGetObjectValue) return
    value.print()
  }
  ...
  override fun visitVariable(declaration: IrVariable) {
    ...
    if (declaration.isLateinit) print("lateinit")
    when {
  ...
  private fun IrClass.correspondingProperty(param: IrValueParameter) =
    declarations
  ...
  override fun visitTypeParameter(declaration: IrTypeParameter) {
    print(declaration.name)
    val isNonEmpty =
      declaration.superTypes.isNotEmpty() &&
  ...
  private inline fun buildTrimEnd(fn: StringBuilder.() -> Unit) =
    buildString(fn).trimEnd()

  private fun IrTypeAbbreviation.renderTypeAbbreviation() =
    buildString {
      append("{ ")
  ...
  private fun renderAsAnnotation(irAnnotation: IrConstructorCall) =
  ...
  private fun IrDeclaration.renderDeclarationParentFqn(sb: StringBuilder) {
    try {
      ...
      if (parent is IrDeclaration) parent.renderDeclarationFqn(sb)
      else if (parent is IrPackageFragment) sb.append(parent.fqName.toString())
